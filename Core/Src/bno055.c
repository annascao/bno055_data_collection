...
void bno055_setCalibrationData(bno055_calibration_data_t calData) {
  ...
  // Assumes little endian processor
  memcpy(buffer, &calData.offset.accel, 6); //copies accelerometer offset to buffer (first index, 6 bytes)
...
bno055_vector_t bno055_getVectorGyroscope() {
  return bno055_getVector(BNO055_VECTOR_GYROSCOPE); // get angular velocity
}
...
bno055_vector_t quaternion_to_euler(bno055_vector_t q) {
	bno055_vector_t euler;

    // pitch
	euler.x = -asin(2 * (q.x * q.z - q.w * q.y));

    // roll
	euler.y = atan2(2.0 * (q.w * q.x + q.y * q.z), q.w * q.w - q.x * q.x - q.y * q.y + q.z * q.z);

    // yaw
	euler.z = atan2(2.0 * (q.x * q.y + q.w * q.z), q.w * q.w + q.x * q.x - q.y * q.y - q.z * q.z);
